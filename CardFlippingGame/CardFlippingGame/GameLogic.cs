...
namespace CardFlippingGame
{
    internal class GameLogic
    {
        ...
        public int SoCoHoi;
        public bool DangCho;// điều kiện ngăn không cho người chơi click(=true cấm click,=false cho click)
        ...
        public void ResetfirstcardVapb1() //reset FirstCard và pb1 
        {
            if (FistCard != null)
            {
                FistCard = null;
        ...
        public void XoaManChoiCu()
        {
            if (CardList.Count > 0)
            {
                foreach(PictureBox c in CardList)
                {
                    f.Controls.Remove(c);
                    c.Dispose();
                }
